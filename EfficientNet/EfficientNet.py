...
class CNNBlock(nn.Module):
    ...
    def forward(self,x):
        return self.siul(self.bn(self.cnn(x)))
...
class EfficientNet(nn.Module):
    def __init__(self,version,num_classes):
        ...
        witdh_factor, depth_factor, dropout_rate = self.calculate_factors(version)
        last_channels = math.ceil(1280 * witdh_factor)
        self.pool = nn.AdaptiveAvgPool2d(1)
        self.features = self.create_features(witdh_factor,depth_factor,last_channels)
            ...
            for layer in range(layers_repeats):
                features.append(
                    InvertedResidualBlock(
                        ...
                        padding= kernel_size//2, #if k=1:pad=0, k=3:pad=1,k=5:2                    )
                    )
                )
